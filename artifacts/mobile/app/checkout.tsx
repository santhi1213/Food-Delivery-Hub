...
//     const deliveryAddress = addresses[selectedAddressIdx]?.address;
//     if (!deliveryAddress) {
...
//         instructions: "Please deliver to the address above",
//       };
...
//         Alert.alert(
//           "Order Placed! 🎉",
//           `Your order has been placed successfully.\nOrder ID: ${response.order.orderId || response.order._id?.slice(-8).toUpperCase()}`,
//           [
//             {
//               text: "Track Order",
//               onPress: () => router.replace(`/order-tracking/${response.order._id}`)
//             }
//           ]
//         );
//       } else {
...
import { api } from "@/lib/api";
import { useColors } from "@/hooks/useColors";
import { initiatePayment, initializeRazorpay } from "@/lib/payment";
...
export default function CheckoutScreen() {
  ...
  const handlePlaceOrder = async () => {
    ...
    try {
      ...
      // 2. If payment is COD, place order directly
      if (selectedPayment === "cod") {
        ...
        Alert.alert(
          ...
          [
            {
              text: "Track Order",
              onPress: () => router.replace(`/order-tracking/${order._id}`)
            }
          ]
        );
        return;
      ...
      // 3. If payment is Razorpay, initiate payment
        ...
        initiatePayment(
          {
            ...
            customerEmail,
            customerPhone,
            description: `Order #${order.orderId || order._id.slice(-8)}`,
          },
          // On Success
          async (paymentResponse) => {
            ...
            try {
              ...
              const verifyResponse = await api.orders.verifyPayment({
                orderId: order._id,
              ...
              [
                {
                  ...
                  onPress: () => {
                    ...
                    api.orders.cancel(order._id).catch(console.error);
                  }
                }
              ]
            );
            ...
            >
              ...
              {method.id === "razorpay" && (
                <View style={[styles.popularTag, { backgroundColor: colors.secondary }]}>
                  <Text style={{ color: colors.primary, fontSize: 10, fontFamily: "Inter_600SemiBold" }}>Secure</Text>
                </View>
              )}
              <View style={[styles.radio, { borderColor: selectedPayment === method.id ? colors.primary : colors.border }]}>
                {selectedPayment === method.id && <View style={[styles.radioFill, { backgroundColor: colors.primary }]} />}
          ...
          {placing ? (
            ...
            <>
              <Text style={[styles.placeBtnText, { color: colors.primaryForeground }]}>
                {selectedPayment === 'razorpay' ? 'Pay & Place Order' : 'Place Order'}
              </Text>
              <Text style={[styles.placeBtnAmount, { color: "rgba(255,255,255,0.85)" }]}>₹{total}</Text>
            </>
          )}
