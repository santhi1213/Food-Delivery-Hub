...
const getBaseUrl = (): string => {
  ...
  if (process.env["EXPO_PUBLIC_DOMAIN"]) {
    // return `https://${process.env["EXPO_PUBLIC_DOMAIN"]}/api`;
    // return "http://localhost:5000/api";
    return "http://192.168.1.74:5000/api";
  }
  ...
  // For physical devices (use your computer's IP)
  // return "http://192.168.1.100:5000/api"; // Change this to your IP
  return "http://192.168.1.74:5000/api"; // Change this to your IP
  //  return "http://localhost:5000/api";
};
...
export const api = {
  ...
  // ==========================================
  // ORDERS ENDPOINTS - COMPLETE FIX
  // ==========================================
  orders: {
    // Create Razorpay payment order (Step 1 - No order saved yet)
    ...
    // Verify payment and create order (Step 2 - Order saved after payment verification)
    verifyAndCreateOrder: (data: {
      razorpayOrderId: string;
      razorpayPaymentId: string;
      razorpaySignature: string;
      orderData: any;
    ...
    // COD order creation (Direct order, no payment verification)
    createCod: (data: {
      restaurantId: string;
      restaurantName: string;
      items: {
